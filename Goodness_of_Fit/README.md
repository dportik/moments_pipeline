# Performing Simulations and Goodness of Fit Tests Using moments

---------------------------------

## Purpose:

Perform goodness of fit tests for demographic models using moments.

This tool is designed to work with the Python package [moments](https://bitbucket.org/simongravel/moments/src/master/) 
and assumes you already have the package installed. This pipeline can be run using Python 2 or 3, but moments is now intended to be run with Python 3 only. You'll need to be familiar with how moments works, 
and some of the basic syntax for writing moments scripts with python. Given the strong similarity between moments and [dadi](https://bitbucket.org/gutenkunstlab/dadi), you can take advantage of the abundance of resources available for dadi, including the [user group](https://groups.google.com/forum/#!forum/dadi-user). Before attempting
to use these scripts, read over the user manual for moments and try running the program with the example files.

## Overview:

This is meant to be a general use script to run moments to perform simulations and goodness of fit tests for any model on an afs/jsfs with one to three populations. To use this workflow, you'll need a SNPs input text file to create an allele frequency or joint site frequency spectrum object. Alternatively, you can import a frequency spectrum of your own creation, editing the script appropriately (see moments manual). The user will have to edit information about their allele frequency spectrum, and a #************** marks lines in the `Simulate_and_Optimize.py` that will have to be edited. 
...
For a general discussion on this workflow (but for dadi), please see the thread [here](https://groups.google.com/forum/#!topic/dadi-user/cjBOopEhIlQ).
...
to that described in the original model-fitting script [here](https://github.com/dportik/moments_pipeline). The routine contains a 
...
***Perform_Sims(sim_number, model_fs, model_name, func, rounds, param_number, fs_folded, reps=None, maxiters=None, folds=None)***
...
100 simulations and define an optimization routine. 
    ...
    Optimize_Functions_GOF.Perform_Sims(sims, scaled_fs, "sym_mig", sym_mig, rounds, p_num, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds)
...
routine will use the default values for each round described [here](https://github.com/dportik/moments_pipeline).
...
## Caveats:
 ...
 single SNP is selected per locus, this is generally true, and this workflow is valid. However, there is also debate about whether we should be using nonparametric bootstraps instead (see the dadi discussion thread [here](https://groups.google.com/forum/#!topic/dadi-user/e7MI1-wU98Q)). To reiterate, this workflow only performs parametric bootstraps, and you will need to decide if this is appropriate for your particular data set.
...
The optimization strategy and the original scripts associated with the dadi version of this pipeline were originally published as part of the following work:
...
The dadi version of the goodness of fit testing scripts were written for the following publication:
...
The moments translation of the above scripts were published as part of:
