...
def Optimize_Routine_GOF(fs, outfile, model_name, func, rounds, param_number, fs_folded=True,
    ...
    #(4) func: access the model function from within script
...
def Optimize_Empirical(fs, outfile, model_name, func, in_params, fs_folded=True):
    #--------------------------------------------------------------------------------------
    # Mandatory Arguments =
    #(1) fs:  spectrum object name
    #(2) outfile:  prefix for output naming
    #(3) model_name: a label to help label the output files; ex. "no_mig"
    #(4) func: access the model function from within script
    #(5) in_params: the previously optimized parameters to use
    #(6) fs_folded: A Boolean value (True or False) indicating whether the empirical fs is folded (True) or not (False). Default is True.
    #--------------------------------------------------------------------------------------
    ...
    #simulate the model with the optimized parameters
    sim_model = func(in_params, fs.sample_sizes)
...
def Perform_Sims(sim_number, model_fs, model_name, func, rounds, param_number, fs_folded=True,
	...
	# Mandatory Arguments =
		...
		#(3) model_name: a label to help label on the output files; ex. "no_mig"
		#(4) func: access the model function from within this script
		#(5) rounds: number of optimization rounds to perform
		#(6) param_number: number of parameters in the model selected (can count in params line for the model)
        #(7) fs_folded: A Boolean value (True or False) indicating whether the empirical fs is folded (True) or not (False). Default is True.
    ...
    with open(sim_out, 'a') as fh_out:
        ...
        for i in range(1,(sims+1)):
            ...
            best_rep = Optimize_Routine_GOF(sim_fs, outfile, model_name, func, rounds, param_number, fs_folded, reps, maxiters)
