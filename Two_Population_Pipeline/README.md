# Two Population Demographic Modeling Pipeline using moments
...
The user will have to edit information about their allele frequency spectrum, and a #************** marks lines in the `moments_Run_2D_Set.py` that will have to be edited. 

The `moments_Run_2D_Set.py`, `Optimize_Functions.py` (from the [main](https://github.com/dportik/moments_pipeline) repository), and `Models_2D.py` scripts must all be in the same working directory for `moments_Run_2D_Set.py` to run properly.

## Available Two Population (2D) Models:

Here is a running list of the models currently available. The name of the model function in the `Models_2D.py` script is given, along with a brief description, and a corresponding visual representation of the model is provided in the file [Models_2D.pdf](https://github.com/dportik/dadi_pipeline/blob/master/Two_Population_Pipeline/Models_2D.pdf). These models were originally written for demographic modeling using dadi, but were translated for use with moments in Leache et al. (2019). If you use these models and scripts for your work, please provide proper citations (provided in the sections below).

The *dadi_pipeline* also included several models related to island diversification from Charles et al. (2018). Many of these island diversification models included aspects involving discrete admixture events. The implementation of admixture events in moments was not so clear and I did not successfully translate these models for moments. If you want to access and use the island diversification models please use the [Two_Population_Pipeline](https://github.com/dportik/dadi_pipeline/tree/master/Two_Population_Pipeline) of the *dadi_pipeline*.
...
Executing the optimization routine multiple times can be accomplished by writing loops in the script, or by running the script multiple times. Here is an example of a custom loop:
    ...
    for i in range(1,6):
        Optimize_Functions.Optimize_Routine(fs, prefix, "sym_mig", Models_2D.sym_mig, rounds, 4, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T")
...
Let's say you no longer wish to include the first two models in the example below. There are three easy options for doing this.
    ...
    Optimize_Functions.Optimize_Routine(fs, prefix, "no_mig", Models_2D.no_mig, rounds, 3, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, T")

    # Split into two populations, with continuous symmetric migration.
    Optimize_Functions.Optimize_Routine(fs, prefix, "sym_mig", Models_2D.sym_mig, rounds, 4, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T")

    # Split into two populations, with continuous asymmetric migration.
    Optimize_Functions.Optimize_Routine(fs, prefix, "asym_mig", Models_2D.asym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T")

    # Split with continuous symmetric migration, followed by isolation.
    Optimize_Functions.Optimize_Routine(fs, prefix, "anc_sym_mig", Models_2D.anc_sym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T1, T2")
...
## Summarizing Outputs Across Models:
 ...
 the path to a folder called *My_Output_files* is specified:
     ...
     python Summarize_Outputs.py /Users/dan/moments_pipeline/TwoPopulationComparisons/My_Output_files
